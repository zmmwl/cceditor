    ...
    _initWS: function () {
        ...
        outputWS.onmessage = function (e) {
            ...
            if ('run' === data.nextCmd) {
                ...
                // alert("executable"+data.executable)
                $.ajax({
                    type: 'POST',
            ...
            if ('run2' === data.nextCmd) {
                var request = newWideRequest();
                request.executable = data.executable;
                request.blockchain = "Y"
                console.log("we are about to run2")
                $.ajax({
                    type: 'POST',
                    url: config.context + '/run',
                    data: JSON.stringify(request),
            ...
            if ('run3' === data.nextCmd) {
                var request = newWideRequest();
                request.executable = data.executable;
                request.blockchain = "Y"
                alert("we are about to run2")
                $.ajax({
                    type: 'POST',
                    url: config.context + '/run',
                    data: JSON.stringify(request),
